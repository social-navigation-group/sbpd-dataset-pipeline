...
class _DetectedInstance:
    ...
    __slots__ = ["label", "bbox", "mask_rle", "color", "ttl","tracking_id"]

    def __init__(self, label, bbox, mask_rle, color, ttl,tracking_id = None):
        self.label = label
        self.bbox = bbox
...
class VideoVisualizer:
    ...
    def draw_instance_predictions(self, frame, predictions, alpha=None):
        ...
        if not predictions.has("COLOR"):
            if predictions.has("ID"):
                ...
                colors,tracking_ids = self._assign_colors(detected)
    ...
    def draw_panoptic_seg_predictions(
        ...
        colors,ids = self._assign_colors(detected)
    ...
    def _assign_colors(self, instances):
        ...
        for idx, inst in enumerate(self._old_instances):
            if max_iou_per_old[idx] > threshold:
                ...
                if instances[newidx].color is None:
                    ...
                    instances[newidx].tracking_id = inst.tracking_id
                    continue
        ...
        for inst in instances:
            if inst.color is None:
                ...
                inst.tracking_id = self._next_tracking_id
                self._next_tracking_id +=1
        self._old_instances = instances[:] + extra_instances
        return [d.color for d in instances],[d.tracking_id for d in instances]

...
def get_args():
    ...
    parser.add_argument("--tracker", default="bytetrack.yaml", type=str, help="Tracker configuration file")
...
def main(args):
    ...
    for video_file in os.listdir(args.video):
        if (not video_file.endswith(".mp4")) and (not video_file.endswith(".avi")):
            continue
        ...
        if video_file.endswith(".mp4"):
            traj_fname = video_file.replace(".mp4", ".toml")
        else:
            traj_fname = video_file.replace(".avi", ".toml")
            ...
            for box in results[0].boxes:
                if box.cls[0] == 0:  # Only track humans (label 0)
                    ...
                    if (frame_id - 1) % interval == 0:
                        ...
                        if (object_id != -1) and (object_id not in id_list):
                            id_list.append(object_id)
                            trajectory_dict[human_name] = {}
                            trajectory_dict[human_name]["traj_start"] = frame_id

...
_via_data.prototype.project_save = function() {
  return new Promise( function(ok_callback, err_callback) {
    try {
      ...
      var filename = [];
      // Use the currently active video filename as base for save if available
      var current_fname = undefined;
      // Try to get the currently active vid from the UI global if available
      if (typeof window !== 'undefined' && window.via && window.via.va && window.via.va.vid) {
        var current_vid = window.via.va.vid;
        if (this.store.view[current_vid] && this.store.view[current_vid].fid_list && this.store.view[current_vid].fid_list.length > 0) {
          var current_fid = this.store.view[current_vid].fid_list[0];
          if (this.store.file[current_fid] && this.store.file[current_fid].fname) {
            current_fname = this.store.file[current_fid].fname.replace(/\.[^/.]+$/, '');
          }
        }
      }
      // fallback to first video if UI context is not available
      if (!current_fname && this.store.project.vid_list && this.store.project.vid_list.length > 0) {
        var fallback_vid = this.store.project.vid_list[0];
        if (this.store.view[fallback_vid] && this.store.view[fallback_vid].fid_list && this.store.view[fallback_vid].fid_list.length > 0) {
          var fallback_fid = this.store.view[fallback_vid].fid_list[0];
          if (this.store.file[fallback_fid] && this.store.file[fallback_fid].fname) {
            current_fname = this.store.file[fallback_fid].fname.replace(/\.[^/.]+$/, '');
          }
        }
      }
      if (current_fname) {
        filename = [current_fname, '.json'];
      } else if ( this.store.project.pid === _VIA_PROJECT_ID_MARKER ) {
        filename = ['via_project.json'];
      } else {
        filename = [this.store.project.pid.substr(0,8) + '.json'];
      }

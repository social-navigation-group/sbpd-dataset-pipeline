...
_via_temporal_segmenter.prototype._init = function() {
  ...
  if ( this.group_aid_candidate_list.length ) {
    ...
    var promise = this.d.attribute_add('TEMPORAL-SEGMENTS',
                                       ...
                                       'Defines the type of temporal segment',
                                       temporal_segment_options,
                                       'Yield');
    ...
    this.d.attribute_add(
      'Scenario Tags',
      'FILE1_Z0_XY0',
      _VIA_ATTRIBUTE_TYPE.CHECKBOX,
      'Scenario tags for the file',
      scenario_tag_options,
      ''
    ).then(function(new_attribute_aid) {
      // attribute added, now update the UI if required
      if(this.fa) {
        this.fa._fmetadata_show();
      }
    }.bind(this), function(err) {
      // Handle error if needed
      console.warn('Failed to add attribute Scenario Tags');
    }.bind(this));
  }
}

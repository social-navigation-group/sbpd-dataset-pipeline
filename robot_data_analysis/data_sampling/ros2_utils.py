...
def get_synced_raw_messages_from_bag(
    ...
    if active_imtopics is None and len(active_masktopics)==0:
    ...
    while bag.has_next():    
        ...
        if currtime is None:
            currtime = t/1e9
        t = t/1e9
        if topic == speechtopics:
            speech_data.update({t:deserialize_message(msg, type_map[topic])})
            continue
        ...
        if topic in active_imtopics:
            ...
            # Use rosbags deserialize_cdr for custom message types
            try:
                curr_maskdata[topic] = deserialize_message(msg, type_map[topic])
            except AttributeError:
                # Fallback to rosbags deserializer for custom types
                if typestore:
                    curr_maskdata[topic] = typestore.deserialize_cdr(msg, 'custom_msgs/msg/BinaryMask')
                else:
                    print(f"Warning: Could not deserialize custom message for topic {topic}")
        ...
        if (t - currtime) >= 1.0 / rate:
            ...
            has_required_images = any(curr_imdata[topic] is not None for topic in active_imtopics) if len(active_imtopics)>0 else True
            ...
            # print(f"Sync check - Images: {has_required_images}, Odom: {has_required_odom}, PCL: {has_required_pcl}, Scan: {has_required_scan}, Tracking: {has_required_tracking}, Masks: {has_required_masks}")
            if (has_required_images and has_required_odom and 
                ...
                currtime = t
                timestamps.append(t)
    return timestamps, synced_imdata, synced_odomdata, synced_pcldata, synced_scandata, synced_trackingdata, synced_maskdata, speech_data
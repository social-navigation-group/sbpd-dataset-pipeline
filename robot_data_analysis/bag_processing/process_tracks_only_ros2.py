...
def process_bag_file(bag_path,config):
    ...
    if len(synced_imdata) == 0:
        raise ValueError(f"No images found in {bag_path}.")
    ...
    if bag_traj_data is None or (bag_pcl_data is None and config[args.dataset_name]["process_pcl"]) or (bag_scan_data is None and config[args.dataset_name]["process_scan"]) or (bag_track_data is None and config[args.dataset_name]["process_tracking"]):
        print(

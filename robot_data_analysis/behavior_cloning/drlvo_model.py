...
class CustomCNN(nn.Module):
    ...
    #
    def __init__(self, map_size = 80, features_dim:int = 256, output_dim: int = 10):
        # network parameters:
        block = Bottleneck
        layers = [2, 1, 1]
        ...
        self.waypoint_predictor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(features_dim//2, output_dim),
        )
    ...
    def _forward_impl(self, ped_pos, scan, goal):
        ###### Start of fusion net ######
        ped_in = ped_pos.reshape(-1,2,self.map_size,self.map_size)
        scan_in = scan.reshape(-1,1,self.map_size,self.map_size)
        ...
        x = self.waypoint_predictor(x)

        return x

    def forward(self, ped_pos: torch.Tensor,scan:torch.Tensor,goal: torch.Tensor) -> torch.Tensor:
        # preprocessing:
